...
class ThrottwinApp {
    ...
    renderDashboardTable() {
        ...
        uniqueDevices.forEach(dev => {
            ...
            const devLabel = dev.hostname || dev.vendor || dev.ip || 'Device';
            const devLabelEscaped = devLabel.replace(/\\/g, '\\\\').replace(/'/g, "\\'").replace(/"/g, '&quot;');
            const wlTitle = isHost ? 'This PC (Permanently Safe)' : (isGlobalWl ? 'Remove from Global Whitelist' : 'Add to Global Whitelist (Never Throttle)');
            const blTitle = isHost ? 'Cannot blacklist host PC' : (isGlobalBl ? 'Remove from Global Blacklist' : 'Add to Global Blacklist (Auto-Target)');
            ...
            tr.innerHTML = `
                ...
                <td style="text-align: right;">
                    <div class="table-actions-group">
                        <button type="button" 
                                class="btn-action-icon btn-action-whitelist ${isGlobalWl ? 'active' : ''}" 
                                onclick="app.toggleDeviceRule('whitelist', '${macLower}', '${devLabelEscaped}')" 
                                title="${wlTitle}"
                                ${isHost ? 'disabled' : ''}>
                            <i class="fa-solid fa-shield-halved"></i>
                        </button>
                        <button type="button" 
                                class="btn-action-icon btn-action-blacklist ${isGlobalBl ? 'active' : ''}" 
                                onclick="app.toggleDeviceRule('blacklist', '${macLower}', '${devLabelEscaped}')" 
                                title="${blTitle}"
                                ${isHost ? 'disabled' : ''}>
    ...
    /* ==========================================================
       7. UTILITIES
       ========================================================== */
    quickAddToRule(mac, vendor, category = 'whitelist') {
        if (!mac || mac === 'Unknown' || mac === '-') {
            this.showToast('Cannot create global rule without a valid MAC address.', 'error');
            return;
        }
        ...
        this.openAddRuleModal(category);
    }

    async toggleDeviceRule(category, mac, name) {
        if (!mac || mac === 'Unknown' || mac === '-' || mac === 'unknown') {
            this.showToast('Cannot set rule without a valid MAC address.', 'error');
            return;
        }

        const macLower = mac.toLowerCase().trim();
        const oppositeCategory = (category === 'whitelist') ? 'blacklist' : 'whitelist';
        const currentRules = this.rules[category] || {};
        const isCurrentlyActive = (macLower in currentRules);

        try {
            if (isCurrentlyActive) {
                // Delete rule (toggle off)
                const res = await fetch(`/api/rules/${category}/${encodeURIComponent(macLower)}`, { method: 'DELETE' });
                const data = await res.json();
                if (data.success) {
                    this.rules = data.rules;
                    this.renderRules();
                    this.renderDashboardTable();
                    this.renderScannerTable();
                    const icon = category === 'whitelist' ? '🛡️' : '🎯';
                    const categoryLabel = category === 'whitelist' ? 'Whitelist' : 'Blacklist';
                    this.showToast(`${icon} Removed ${name || macLower} from Global ${categoryLabel}`, 'info');
                } else {
                    this.showToast(data.error || 'Failed to update rule.', 'error');
                }
            } else {
                // Add rule (toggle on)
                // If it exists in opposite category, delete from opposite first to avoid conflict
                if (this.rules[oppositeCategory] && (macLower in this.rules[oppositeCategory])) {
                    await fetch(`/api/rules/${oppositeCategory}/${encodeURIComponent(macLower)}`, { method: 'DELETE' });
                }

                const res = await fetch('/api/rules', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ category, mac: macLower, name: name || '' })
                });
                const data = await res.json();
                if (data.success) {
                    this.rules = data.rules;
                    ...
                    const icon = category === 'whitelist' ? '🛡️' : '💀';
                    const categoryLabel = category === 'whitelist' ? 'Whitelist' : 'Blacklist';
                    this.showToast(`${icon} Added ${name || macLower} to Global ${categoryLabel}`, 'success');
                } else {
                    this.showToast(data.error || 'Failed to save rule.', 'error');
                }
            }
        } catch (e) {
            console.error('Error toggling device rule:', e);
            this.showToast('Network error while updating rules.', 'error');
        }
    }
    ...
    openAddRuleModal(category) {
        ...
        const title = isWl ? 'Add to Global Whitelist' : 'Add to Global Blacklist';
        const titleEl = document.getElementById('addRuleModalTitle');
        if (titleEl) {
            const iconClass = isWl ? 'fa-shield-halved emerald' : 'fa-skull-crossbones rose';
            titleEl.innerHTML = `<i class="fa-solid ${iconClass}"></i> ${title}`;
        }
