...
def resolve_hostname(ip, timeout=0.25, gateway=None):
    """Aggressive reverse DNS and multi-vector device name resolution."""
    if not ip or ip in ("-", "Unknown", "0.0.0.0"):
        return ""
    try:
        from .hostname import resolve_device_name_aggressive
        return resolve_device_name_aggressive(ip, gateway=gateway)
    except Exception:
        pass
    return ""
